...
const generateLabelText = (city?: string) =>
    city ? `Flights from ${city}` : "";
...
const getItemByMainCityTitle = () =>
    mapPointSeriesData.value.find(({ title }) =>
        compareStrings(title, mainCity.value)
    );
        ...
        return am5.Bullet.new(root, {
            sprite: am5.Graphics.new(
                root,
                {
                    ...
                    fill: am5.color(
                        isMainCity ? colors.primary : itemData.color
                    ),
...
const updateInteractiveElements = () => {
    mapLabelText.value.set(
        "text",
        generateLabelText(getItemByMainCityTitle()?.title)
    );
    mapHomeCityButton.value[
        compareStrings(props.homeCity, mainCity.value) ? "hide" : "show"
    ]?.();
};
