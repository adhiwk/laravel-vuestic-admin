    ...
    <va-card class="d-flex dashboard-contributors-list">
        ...
        <va-card-content>
            <va-inner-loading :loading="loading" style="width: 100%">
                ...
                >
                    <va-progress-bar
                        :model-value="getPercent(contributor.contributions)"
                        :color="getProgressBarColor(idx)"
                    >
                        {{ contributor.contributions }}
                        {{ t("dashboard.charts.commits") }}
                    </va-progress-bar>
...
async function loadContributorsList() {
    ...
    progressMax.value = Math.max(
        ...contributors.value.map(({ contributions }) => contributions)
    );
    showNext();
    loading.value = false;
}
...
function showNext() {
    visibleList.value = contributors.value.slice(
        page.value * step.value,
        page.value * step.value + step.value
    );
    page.value += 1;
...
function getProgressBarColor(idx: number) {
    ...
    return themeColors[
        keys[(keys.length * Math.random()) << 0] as unknown as number
    ];
}
</script>
